...
export default function Pagination({ onPaginationClick, page, totalPages }) {
  const getPagination = (page, totalPages, count) => {
    ...
    if (totalPages <= count) {
      for (let i = 1; i <= totalPages; i++) {
        arr.push(
          <li className={i == +page ? "pagination__active" : ""} key={i}>
            {i}
          </li>
        );
      }
  ...
  const onClick = useCallback(
    event => {
      if (event.target.tagName == "LI") {
        onPaginationClick(event.target.innerText);
      }
    },
    [onPaginationClick]
  );
  ...
  return (
    <div className="pagination-wrapper">
      <ul onClick={onClick} className="pagination">
